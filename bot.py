...
def generate_chart(candles, detection, levels, title="", filename="/tmp/chart_setup.png"):
    ...
    try:
        ...
        # Líneas horizontales: niveles SD + entrada (zona OTE) + SL
        hlines_prices = [
            levels["entry"], levels["sl"],
            levels.get("entry_cons", levels["entry"]),
            levels.get("entry_agg", levels["entry"]),
            levels["sd_m1"], levels["sd_m2"], levels["sd_m25"], levels["sd_m4"],
        ]
        hlines_colors = [
            "#FFD700", "#FF4444",       # entrada óptima (oro), SL (rojo)
            "#FFA500", "#FFCC00",       # PRO 10: cons (naranja), agg (amarillo)
            "#888888", "#00CC66", "#00AA55", "#008844",  # SDs
        ]
        hlines_styles = [
            "-", "--",           # entrada óptima sólida, SL dashed
            ":", ":",            # PRO 10: cons y agg punteadas
            ":", "-", "-", ":",  # SDs
        ]
        hlines_widths = [1.2, 1.2, 0.7, 0.7, 0.6, 0.9, 0.9, 0.6]

        # Banda del rango de acumulación + zona OTE
        zone_high = levels.get("zone_high")
        zone_low  = levels.get("zone_low")
        if zone_high and zone_low:
            # Dos bandas: rango (azul) + zona OTE (amarilla translúcida)
            fill = [
                dict(y1=acc_low, y2=acc_high, alpha=0.15, color="#4488FF"),
                dict(y1=zone_low, y2=zone_high, alpha=0.12, color="#FFD700"),
            ]
        else:
...
def send_test_message():
    ...
    send_telegram("🧪 <b>TEST v28 — Alerta 1</b>\n\n" +
                  format_detection_alert("BTCUSDT", "4H", detection, levels, score))
    time.sleep(2)
    send_telegram("🧪 <b>TEST v28 — Alerta 2: CHoCH confirmado</b>\n\n" +
...
# ─── MAIN ────────────────────────────────────────────────────
def main():
    print("🤖 Bot TTrades v28 — PRO Edition")
    ...
    send_telegram(
        "🤖 <b>Bot TTrades AMD v28 — PRO Edition + CHoCH Retroactivo</b>\n\n"
