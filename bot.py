...
def detect_manipulation(candles, interval, vol_percentile=50.0, adaptive_mult=None):
    ...
    # ── v36 Diferencia #2: Verificar SWEEP de liquidez real ───
...
def detect_live_spike(candles, interval, acc_high_cache=None, acc_low_cache=None):
    ...
    if live_size < avg_size * PRE_ALERT_SIZE_MIN:  # v36: umbral configurable
        return None
...
def calculate_levels(detection):
    ...
    # v36 Diferencia #7: SL basado en ATR (buffer = 20% del rango del leg)
...
def quality_score(confs, rr, macro_bias, mt, detection, dynamic_weights=None,
    ...
    """
    v36: Scoring reescrito según el PDF de TTrades (Cap. 12.3).
...
def send_test_message():
    ...
    send_telegram("🧪 <b>TEST v36 — Alerta 1</b>\n\n" +
                  format_detection_alert("BTCUSDT", "4H", detection, levels, score))
    time.sleep(2)
    send_telegram("🧪 <b>TEST v36 — Alerta 2: CISD confirmado</b>\n\n" +
...
# ─── MAIN ────────────────────────────────────────────────────
def main():
    print("🤖 Bot TTrades v36 — PRO Edition")
    ...
    send_telegram(
        "🤖 <b>Bot TTrades AMD v36 — PRO Edition + CISD Retroactivo</b>\n\n"
        ...
        "📐 v36 — ALINEACIÓN COMPLETA CON PDF TTRADES:\n"
        ...
        for sym in SYMBOLS:
            ...
            for tf in TIMEFRAMES:
                ...
                try:
                    ...
                    # ── v36 DIFERENCIA #1: FILTRO HTF OBLIGATORIO ─────
                    ...
                    # ── v36 DIFERENCIA #3: 3 CONDICIONES MÍNIMAS ──────
                    ...
                    # ── v36 DIFERENCIA #10: ANTI-CONTRADICCIÓN ────────
                    contradiction = False
                    ...
                    # v36: scoring con pesos del PDF + PD Arrays + funding
                    ...
                    # v36 Diferencia #12: STACKING de proyecciones SD
                    stacking_bonus = 0
                    for pk, ps in pending_setups.items():
                        if ps["sym"] == sym and ps["interval"] != interval and \
            ...
            for setup_key, setup in pending_setups.items():
                ...
                if setup["choch_alerted"]:
                    ...
                    if not setup["sl_hit"]:
                        ...
                        # v36 Diferencia #6: Partials 50/30/20 del PDF
                        tp_config = [
                        ...
                        if sl_hit:
                            ...
                            for tp_key, tp_price, tp_label, tp_advice in tp_config:
                                ...
                                if tp_reached:
                                    ...
                                    # v36 Diferencia #9: Displacement check en -2.5
                                    if tp_key == "sd_m25":
                                        try:
