...
def quality_score(confs, rr, macro_bias, mt, detection):
    ...
    # MEJORA 4: scoring por tipo de retorno (completo vs parcial)
    return_type = detection.get("return_type", "NINGUNO")
    if return_type == "COMPLETO":   score += 12
    elif return_type == "PARCIAL":  score += 6
    # MEJORA 3: scoring por nivel de liquidez (reemplaza el binario has_liq)
    liq_level = detection.get("liq_level", "BAJA")
    if liq_level == "ALTA":     score += 12
    elif liq_level == "NORMAL": score += 8
    elif detection.get("has_liq"): score += 4
    # Bonus si hay Equal Highs/Lows barridos
    if detection.get("has_eqhl"): score += 5
    if rr >= 2.5:   score += 15
...
def send_test_message():
    ...
    send_telegram("🧪 <b>TEST v15 — Alerta 1</b>\n\n" +
                  format_detection_alert("BTCUSDT", "4H", detection, levels, score))
    time.sleep(2)
    send_telegram("🧪 <b>TEST v15 — Alerta 2: CHoCH confirmado</b>\n\n" +
                  format_choch_alert("BTCUSDT", "4H", detection, levels, confluences,
                                     "BEARISH", "BEARISH", score,
                                     detection["swing_choch"] * 0.999, price * 0.997))
    print("✅ Test v15 enviado")

# ─── MAIN ────────────────────────────────────────────────────
def main():
    print("🤖 Bot TTrades v15 — Rango dinámico + Spike adaptativo + Liquidez + Retorno proporcional")
    ...
    send_telegram(
        "🤖 <b>Bot TTrades AMD v15 activado</b>\n\n"
        "🆕 Mejora 1: Rango acumulacion dinamico por ATR\n"
        "🆕 Mejora 2: Umbral spike adaptativo por volatilidad\n"
        "🆕 Mejora 3: Filtro de liquidez real (toques + EQH/EQL)\n"
        "🆕 Mejora 4: Retorno al rango proporcional (30% duracion)\n\n"
