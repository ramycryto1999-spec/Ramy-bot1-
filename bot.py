...
def detect_live_spike(candles, interval, acc_high_cache=None, acc_low_cache=None):
    ...
    if live_size < avg_size * 1.5:  # umbral más bajo para detección temprana
        return None

    # Volumen alto
    avg_vol = sum(c["volume"] for c in prev[-LOOKBACK:]) / LOOKBACK
    if avg_vol <= 0 or live["volume"] / avg_vol < 1.5:
        return None
...
def _entry_recommendation(score, swing, direction):
    ...
    if score >= 75:
        return (
            ...
            "  → No necesitas esperar CHoCH para entrar\n"
            "  → El CHoCH te llegará como confirmación bonus\n\n"
            "💡 <i>Setup de alta probabilidad. Actúa rápido.</i>"
        )
    elif score >= 50:
        return (
            f"⏳ <b>Esperar CHoCH en 15M para confirmar</b>\n"
        ...
        return (
            f"⚠️ <b>Setup débil — Solo entrar con CHoCH FUERTE</b>\n"
            f"  Nivel a romper: <b>${swing:,.2f}</b>\n"
            f"  Condicion: cierre {direction} de ${swing:,.2f}\n\n"
...
{choch_str} <b>Calidad CHoCH:</b> cuerpo >50% + cierre tercio favorable{sweep_line}
...
def send_test_message():
    ...
    send_telegram("🧪 <b>TEST v33 — Alerta 1</b>\n\n" +
                  format_detection_alert("BTCUSDT", "4H", detection, levels, score))
    time.sleep(2)
    send_telegram("🧪 <b>TEST v33 — Alerta 2: CHoCH confirmado</b>\n\n" +
...
# ─── MAIN ────────────────────────────────────────────────────
def main():
    print("🤖 Bot TTrades v33 — PRO Edition")
    ...
    send_telegram(
        "🤖 <b>Bot TTrades AMD v33 — PRO Edition + CHoCH Retroactivo</b>\n\n"
        ...
        "🆕 PRO 9: CHoCH retroactivo (movimientos agresivos)\n"
        ...
        "🆕 PRO 14: CHoCH condicional (directo/esperar/fuerte)\n"
    ...
    while True:
        ...
        if time.time() - last_heartbeat > HEARTBEAT_INTERVAL:
            ...
            send_telegram(
                ...
                f"  Setups pendientes: {pending_count} ({choch_count} post-CHoCH)\n"
        ...
        for sym in SYMBOLS:
            ...
            for tf in TIMEFRAMES:
                ...
                try:
                    ...
                    if setup_key not in pending_setups:
                        ...
                        if score >= 75:
                            entry_mode = "DIRECTA"
                            choch_done = True   # no esperar CHoCH
                            ...
                            choch_done = False
                            entry_price = None
                            print(f"  ⏳ Esperar CHoCH — {label} {sym} | Score {score} | Swing ${detection['swing_choch']:,.2f}")
                        else:
                            entry_mode = "SOLO_CHOCH_FUERTE"
                            choch_done = False
                            entry_price = None
                            print(f"  ⚠️ Solo CHoCH fuerte — {label} {sym} | Score {score}")
            ...
            for setup_key, setup in pending_setups.items():
                ...
                if not setup["choch_alerted"] and current_price:
                    ...
                    if fib0_reached:
                        # PRO 9: ANTES de descartar, verificar CHoCH retroactivo
                        ...
                        try:
                            ...
                            print(f"  ⚠️ Error check CHoCH retroactivo: {e}")
                        ...
                        if retroactive_choch:
                            ...
                            send_telegram(
                                f"⚡ <b>CHoCH RETROACTIVO DETECTADO</b> — {sym} {setup['label']}\n\n"
                                f"El precio se movió tan rápido que llegó al nivel 0 "
                                f"antes del ciclo de monitoreo, pero <b>SÍ hubo CHoCH</b> "
                                f"válido en 15M ({time_str}).\n\n"
                                f"  CHoCH en:      ${retro_price:,.2f}\n"
                            ...
                            print(f"  ⚡ CHoCH retroactivo detectado en ${retro_price:,.2f}")

                            # Registrar en Sheets como setup confirmado (no como descartado)
                            if SHEETS_ENABLED:
                                try:
                                    update_choch(setup_key, retro_price,
                                                setup["confluences"] + ["CHoCH RETROACTIVO"],
                                                current_price,
                                                setup["macro_bias"], setup["detection"]["type"])
                                except Exception as e:
                                    print(f"  ⚠️ Error Sheets CHoCH retroactivo: {e}")

                            expired_keys.append(setup_key)
                            continue

                        # Sin CHoCH retroactivo → descarte normal
                        expired_keys.append(setup_key)
                        send_telegram(
                        ...
                        print(f"  📉 Descartado — precio alcanzó fib0 ${fib0:,.2f} sin CHoCH (ni retroactivo)")
                        continue
                ...
                if setup["choch_alerted"]:
                    ...
                    # PRO 14: bonus CHoCH para entradas directas
                    if setup.get("entry_mode") == "DIRECTA" and not setup.get("choch_bonus_sent"):
                        choch_ok, choch_p, choch_q = check_choch_15m(sym, setup["detection"])
                        if choch_ok:
                            ...
                            send_telegram(
                                f"✅ <b>CHoCH CONFIRMADO (bonus)</b> — {sym} {setup['label']}\n\n"
                                f"  Calidad: {q_str}\n"
                                f"  CHoCH en: ${choch_p:,.2f}\n"
                                f"  Precio actual: ${current_price:,.2f}\n\n"
                                f"💡 <i>Tu entrada directa queda confirmada por CHoCH.\n"
                                f"Mantener posición con confianza.</i>"
                            )
                            print(f"  ✅ Bonus CHoCH {choch_q} — {sym} en ${choch_p:,.2f}")
                            if SHEETS_ENABLED:
                                try:
                ...
                if choch_confirmed and current_price:
                    ...
                    if entry_mode == "SOLO_CHOCH_FUERTE" and choch_quality != "FUERTE":
                        send_telegram(
                            f"⚠️ <b>CHoCH DEBIL RECHAZADO</b> — {sym} {setup['label']}\n\n"
                            f"Score {setup['score']}/100 — requiere CHoCH fuerte.\n"
                            f"  Precio ruptura: ${choch_price:,.2f}\n"
                            f"  Precio actual:  ${current_price:,.2f}\n\n"
                            f"⚠️ <i>Setup débil + CHoCH débil = no entrar.</i>"
                        )
                        print(f"  ⚠️ CHoCH débil rechazado para setup débil — {sym}")
