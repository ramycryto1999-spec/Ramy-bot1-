...
# ─── v37: FILTROS ESTRICTOS DEL PDF ─────────────────────────
# Diferencia #1: filtro HTF obligatorio
HTF_FILTER_ENABLED = True       # bloquear setups contra tendencia HTF

# Diferencia #3: 3 condiciones mínimas obligatorias
REQUIRE_HTF_NARRATIVE  = True   # necesita sesgo HTF claro
REQUIRE_SWEEP          = True   # necesita barrer swing previo
REQUIRE_ACTIVE_SESSION = True   # necesita sesión activa
ACTIVE_SESSIONS = ("Londres", "Nueva York", "Overlap", "Pre-Londres")

# Diferencia #8: funding rate
FUNDING_EXTREME    = 0.05       # 0.05% cada 8h = extremo
FUNDING_PENALTY    = 15         # puntos de penalización

# Diferencia #10: anti-contradicción
ANTI_CONTRADICTION_HOURS = 6    # ventana de contradicción

# Diferencia #11: pre-alertas más estrictas
PRE_ALERT_VOL_MIN  = 2.5       # volumen mínimo para pre-alertas
PRE_ALERT_SIZE_MIN = 2.5       # tamaño de vela mínimo para pre-alertas
...
def get_funding_rate(symbol):
    """Obtiene el funding rate actual de Binance para perpetuos."""
    cache_key = f"funding_{symbol}"
    with _api_lock:
        if cache_key in _api_cache:
            ts, data = _api_cache[cache_key]
            if time.time() - ts < 300:  # cache 5 min
                return data
        _check_rate_limit()
    try:
        r = requests.get("https://fapi.binance.com/fapi/v1/premiumIndex",
                         params={"symbol": symbol}, timeout=5)
        data = r.json()
        rate = float(data.get("lastFundingRate", 0)) * 100  # convertir a %
        with _api_lock:
            _api_cache[cache_key] = (time.time(), rate)
        return rate
    except:
        return 0.0
...
def detect_manipulation(candles, interval, vol_percentile=50.0, adaptive_mult=None):
    ...
    closed = get_closed(candles, interval)
    if len(closed) < LOOKBACK + 3:
        return None

    # MEJORA 1: rango dinámico por ATR (usar última vela como referencia inicial)
    ref_index = len(closed) - 1
    prev_candles_fallback = closed[-(LOOKBACK + 1):-1]
    acc_high, acc_low, range_num_candles, compression_level = get_accumulation_range(
        prev_candles_fallback, all_candles=closed, spike_index=ref_index
    )
    acc_range = acc_high - acc_low
    if acc_range <= 0:
        return None

    range_start = max(0, ref_index - range_num_candles)
    prev_candles = closed[range_start:ref_index]
    if not prev_candles:
        prev_candles = prev_candles_fallback

    avg_size = sum(c["total_size"] for c in prev_candles) / len(prev_candles)

    # v37 CORRECCIÓN 3: Manipulation leg multi-vela
    # Buscar entre las últimas 3 velas la que tenga el extremo más pronunciado
    # que perfore el rango de acumulación con tamaño suficiente.
    manip = None
    spike_index = None
    
    best_high_idx = max(range(len(closed) - 3, len(closed)), key=lambda i: closed[i]["high"])
    best_low_idx  = min(range(len(closed) - 3, len(closed)), key=lambda i: closed[i]["low"])
    
    for try_idx in [best_high_idx, best_low_idx, len(closed) - 1]:
        try_candle = closed[try_idx]
        try_size = try_candle["total_size"]
        if try_size < avg_size * spike_multiplier:
            continue
        if try_candle["high"] > acc_high or try_candle["low"] < acc_low:
            manip = try_candle
            spike_index = try_idx
            break
    ...
    # Recalcular prev_candles con spike_index real (si difiere)
    if spike_index != ref_index:
        range_start = max(0, spike_index - range_num_candles)
        prev_candles = closed[range_start:spike_index]
        if not prev_candles:
            prev_candles = prev_candles_fallback

    # v37 CORRECCIÓN 2: body_pct eliminado
    # El PDF no requiere cuerpo mínimo. Los sweeps con wick largo
    # y cuerpo pequeño (hammer/doji) son manipulaciones válidas.
    # Lo que importa es que barre liquidez, no la forma del cuerpo.
    ...
    # ── v37 Diferencia #2: Verificar SWEEP de liquidez real ───
    # El spike debe barrer al menos 1 swing high/low previo.
    if REQUIRE_SWEEP:
        lookback_swings = closed[max(0, spike_index - 30):spike_index]
        if len(lookback_swings) >= 5:
            swings_check = find_swing_points(lookback_swings, lookback=2)
            swept_any = False
            if manip_type == "BAJISTA":
                for _, sh_price in swings_check["highs"]:
                    if manip["high"] > sh_price:
                        swept_any = True
                        break
            else:
                for _, sl_price in swings_check["lows"]:
                    if manip["low"] < sl_price:
                        swept_any = True
                        break
            if not swept_any:
                return None  # no barre ningún swing = no es manipulación
    ...
    # ── v37 CORRECCIÓN 1: CISD LEVEL PRE-MANIPULACIÓN ─────────
    # Buscar la serie de velas que PRECEDIERON al spike (la delivery)
    # y marcar el OPEN de la PRIMERA vela de esa serie.
    # Ese es el nivel que el CISD debe cruzar.
    cisd_level = None
    if manip_type == "BAJISTA":
        # Setup bajista: buscamos serie de velas ALCISTAS antes del spike
        # Recorremos las velas previas de atrás hacia adelante buscando
        # la serie de bullish candles que llevó al high
        pre = list(reversed(prev_candles))  # más reciente primero
        series_end = None
        for i, c in enumerate(pre):
            if c["is_bullish"]:
                series_end = i  # marca la más reciente de la serie
            else:
                if series_end is not None:
                    # Encontramos el fin de la serie alcista — ahora buscamos el inicio
                    series_start_idx = series_end
                    for j in range(series_end, len(pre)):
                        if pre[j]["is_bullish"]:
                            series_start_idx = j
                        else:
                            break
                    # El OPEN de la primera alcista (la más antigua de la serie)
                    cisd_level = pre[series_start_idx]["open"]
                    break
                break  # si la primera vela pre-spike no es bullish, salimos
        if cisd_level is None and prev_candles:
            # Fallback: usar el open de la primera vela alcista de las últimas 5
            for c in reversed(prev_candles[-5:]):
                if c["is_bullish"]:
                    cisd_level = c["open"]
                    break
            if cisd_level is None:
                cisd_level = min(c["low"] for c in prev_candles[-5:])
    else:
        # Setup alcista: buscamos serie de velas BAJISTAS antes del spike
        pre = list(reversed(prev_candles))
        series_end = None
        for i, c in enumerate(pre):
            if not c["is_bullish"]:
                series_end = i
        ...
        if cisd_level is None and prev_candles:
            for c in reversed(prev_candles[-5:]):
                if not c["is_bullish"]:
                    ...
                    break
            if cisd_level is None:
                cisd_level = max(c["high"] for c in prev_candles[-5:])

    # Swing previo (legacy, mantener para compatibilidad)
    swing_choch = cisd_level if cisd_level else (
        min(c["low"] for c in prev_candles[-5:]) if manip_type == "BAJISTA"
        else max(c["high"] for c in prev_candles[-5:])
    )
    ...
    return {
        ...
        "cisd_level":    cisd_level,  # v37: nivel CISD pre-manipulación
        "fib_1":         fib_1,
        "fib_0":         fib_0,
        # MEJORA 1
...
def detect_live_spike(candles, interval, acc_high_cache=None, acc_low_cache=None):
    ...
    if live_size < avg_size * PRE_ALERT_SIZE_MIN:  # v37: umbral configurable
        return None

    # Volumen alto
    avg_vol = sum(c["volume"] for c in prev[-LOOKBACK:]) / LOOKBACK
    if avg_vol <= 0 or live["volume"] / avg_vol < PRE_ALERT_VOL_MIN:
        return None
...
def check_cisd_15m(symbol, detection):
    """
    v37 CORRECCIÓN 1: CISD correcto según el PDF de TTrades (Cap. 3).
    
    El nivel CISD viene pre-calculado en detection["cisd_level"]:
    es el OPEN de la PRIMERA vela de la serie de delivery que
    PRECEDIÓ al spike. Solo verificamos si alguna vela 15M post-spike
    cerró más allá de ese nivel.
    """
    ...
    cisd_level = detection.get("cisd_level")
    if cisd_level is None:
        return False, None, None
    ...
    for c in post:
        confirmed = False
        if manip_type == "BAJISTA" and c["close"] < cisd_level:
            confirmed = True
        elif manip_type == "ALCISTA" and c["close"] > cisd_level:
            confirmed = True
...
def calculate_levels(detection):
    ...
    # v37 Diferencia #7: SL basado en ATR (buffer = 20% del rango del leg)
    sl_buffer = rango * 0.20
    ...
    if mt == "BAJISTA":
        ...
        entry      = fib_1 - rango * 0.705
        entry_agg  = fib_1 - rango * 0.79
        sl     = fib_1 + sl_buffer   # encima del spike + buffer
        ...
        entry      = fib_1 + rango * 0.705
        entry_agg  = fib_1 + rango * 0.79
        sl     = fib_1 - sl_buffer   # debajo del spike - buffer
        sd_m1  = fib_0 + rango * 1.0
        sd_m2  = fib_0 + rango * 2.0
        sd_m25 = fib_0 + rango * 2.5
        sd_m4  = fib_0 + rango * 4.0
...
def quality_score(confs, rr, macro_bias, mt, detection, dynamic_weights=None,
                  levels=None, fvgs=None, obs=None, funding_rate=0.0):
    """
    v37: Scoring reescrito según el PDF de TTrades (Cap. 12.3).
    Base: 120 puntos. Umbral mínimo: 50.
    """
    score = 0

    # 1. CISD claro (+10) — se asume confirmado al llegar aquí
    score += 10

    # 2. Manipulation leg barre liquidez relevante (+15)
    swept_pct = 0
    liq_level = detection.get("liq_level", "BAJA")
    if liq_level == "ALTA":
        score += 15
    ...
    # 3. SD -2 cae en FVG HTF no mitigado (+25)
    sd_in_fvg = False
    if levels and fvgs:
        tp1 = levels.get("sd_m2", 0)
        for fvg in fvgs:
            fvg_top = fvg.get("top", 0)
            fvg_bot = fvg.get("bottom", 0)
            if fvg_bot <= tp1 <= fvg_top:
                sd_in_fvg = True
                score += 25
                break

    # 4. SD -2.5 cae en OB HTF o EQH/EQL (+20)
    sd_in_ob = False
    if levels and obs:
        tp2 = levels.get("sd_m25", 0)
        tol = levels.get("rango", 0) * 0.15
        for ob in obs:
            ob_top = ob.get("top", 0)
            ob_bot = ob.get("bottom", 0)
            if ob_bot - tol <= tp2 <= ob_top + tol:
                sd_in_ob = True
                score += 20
                break
    if detection.get("has_eqhl"):
        score += 5  # bonus parcial por EQH/EQL

    # 5. SD -4 cae en PD Array opuesto (+15)
    sd4_in_pd = False
    if levels and (fvgs or obs):
        sd4 = levels.get("sd_m4", 0)
        tol = levels.get("rango", 0) * 0.20
        all_pd = (fvgs or []) + (obs or [])
        for pd in all_pd:
            pd_top = pd.get("top", 0)
            pd_bot = pd.get("bottom", 0)
            if pd_bot - tol <= sd4 <= pd_top + tol:
                sd4_in_pd = True
                score += 15
                break

    # 6. Funding extremo aliado al setup (+10)
    if abs(funding_rate) >= FUNDING_EXTREME:
        if (mt == "BAJISTA" and funding_rate > 0) or \
           (mt == "ALCISTA" and funding_rate < 0):
            score += 10  # funding aliado
        else:
            score -= FUNDING_PENALTY  # funding en contra = penalización

    # 7. Sesión activa (+10)
    session = detection.get("session", "Post-NY")
    if session in ("Londres", "Nueva York", "Overlap"):
        score += 10
    elif session in ("Pre-Londres", "Asia"):
        score += 4

    # 8. Doble timeframe alineado (+15) — se añade externamente
    # (multi_tf_bonus en el main loop)

    # 9. Volumen del spike (+8 bonus)
    spike_vol = detection.get("spike_vol_level", "")
    if spike_vol == "ALTO":
        score += 8
    elif spike_vol == "CONFIRMADO":
        score += 4
    ...
    # 11. Bias HTF alineado (+5 bonus)
    aligned = (macro_bias == "BULLISH" and mt == "ALCISTA") or \
              (macro_bias == "BEARISH" and mt == "BAJISTA")
    if aligned:
        score += 5

    # Normalizar a 100 (máx teórico ~128)
    score = min(round(score * 100 / 120), 100)
    return max(score, 0)
...
def format_detection_alert(symbol, tf_label, detection, levels, score, liq_analysis=None):
    ...
    swing      = detection.get("cisd_level") or detection["swing_choch"]
...
def send_test_message():
    ...
    send_telegram("🧪 <b>TEST v37 — Alerta 1</b>\n\n" +
                  format_detection_alert("BTCUSDT", "4H", detection, levels, score))
    time.sleep(2)
    send_telegram("🧪 <b>TEST v37 — Alerta 2: CISD confirmado</b>\n\n" +
...
# ─── MAIN ────────────────────────────────────────────────────
def main():
    print("🤖 Bot TTrades v37 — PRO Edition")
    ...
    send_telegram(
        "🤖 <b>Bot TTrades AMD v37 — PRO Edition + CISD Retroactivo</b>\n\n"
        ...
        "📐 v37 — ALINEACIÓN COMPLETA CON PDF TTRADES:\n"
        "  ✅ Filtro HTF obligatorio (no contra-tendencia)\n"
        "  ✅ Sweep de liquidez verificado (swing real)\n"
        "  ✅ 3 condiciones mínimas obligatorias\n"
        "  ✅ Scoring PDF (PD Arrays en SD levels)\n"
        "  ✅ Partials 50/30/20\n"
        "  ✅ SL basado en ATR\n"
        "  ✅ Funding rate filter\n"
        "  ✅ Displacement check -2.5\n"
        "  ✅ Anti-contradicción por par\n"
        "  ✅ Pre-alertas 2.5x\n"
        "  ✅ Stacking SD multi-TF\n\n"
        "🔧 v37 — CORRECCIONES CRÍTICAS PDF:\n"
        "  ✅ CISD busca serie PRE-spike (no post)\n"
        "  ✅ body_pct eliminado (sweeps wick largo)\n"
        "  ✅ Manipulation leg multi-vela (3 últimas)\n\n"
        ...
        for sym in SYMBOLS:
            ...
            for tf in TIMEFRAMES:
                ...
                try:
                    ...
                    # ── v37 DIFERENCIA #1: FILTRO HTF OBLIGATORIO ─────
                    if HTF_FILTER_ENABLED and interval != "1d":  # Daily ES el HTF, no filtramos contra sí mismo
                        if macro_bias == "BULLISH" and detection["type"] == "BAJISTA":
                            print(f"  ❌ {label} {sym}: setup BAJISTA contra tendencia BULLISH HTF — bloqueado")
                            alerted[key] = last_closed_time
                            continue
                        if macro_bias == "BEARISH" and detection["type"] == "ALCISTA":
                            print(f"  ❌ {label} {sym}: setup ALCISTA contra tendencia BEARISH HTF — bloqueado")
                            alerted[key] = last_closed_time
                            continue

                    # ── v37 DIFERENCIA #3: 3 CONDICIONES MÍNIMAS ──────
                    session_name = detection.get("session", "Post-NY")

                    if REQUIRE_HTF_NARRATIVE and macro_bias == "NEUTRAL" and interval != "1d":
                        print(f"  ❌ {label} {sym}: sin narrativa HTF clara — bloqueado")
                        alerted[key] = last_closed_time
                        continue

                    if REQUIRE_ACTIVE_SESSION and session_name not in ACTIVE_SESSIONS and interval != "1d":
                        print(f"  ❌ {label} {sym}: sesión {session_name} no activa — bloqueado")
                        alerted[key] = last_closed_time
                        continue

                    # ── v37 DIFERENCIA #10: ANTI-CONTRADICCIÓN ────────
                    contradiction = False
                    for pk, ps in pending_setups.items():
                        if ps["sym"] == sym and ps["detection"]["type"] != detection["type"]:
                            hours_diff = (now - ps["detected_at"]).total_seconds() / 3600
                            if hours_diff < ANTI_CONTRADICTION_HOURS:
                                print(f"  ❌ {label} {sym}: contradicción con {ps['label']} ({ps['detection']['type']}) hace {hours_diff:.1f}h — bloqueado")
                                contradiction = True
                                break
                    if contradiction:
                        alerted[key] = last_closed_time
                    ...
                    # v37 Diferencia #12: STACKING de proyecciones SD
                    stacking_bonus = 0
                    for pk, ps in pending_setups.items():
                        if ps["sym"] == sym and ps["interval"] != interval and \
                           ps["detection"]["type"] == detection["type"] and "levels" in ps:
                            other_lvls = ps["levels"]
                            rng_tol = levels["rango"] * 0.30
                            # Check si SD -2 de un TF coincide con -4 del otro (o viceversa)
                            checks = [
                                (levels["sd_m2"], other_lvls.get("sd_m4")),
                                (levels["sd_m4"], other_lvls.get("sd_m2")),
                                (levels["sd_m2"], other_lvls.get("sd_m25")),
                                (levels["sd_m25"], other_lvls.get("sd_m2")),
                            ]
                            for my_sd, other_sd in checks:
                                if other_sd and abs(my_sd - other_sd) <= rng_tol:
                                    stacking_bonus = 12
                                    confluences.append(f"Stacking SD: {label} y {ps['label']} convergen en ${my_sd:,.2f}")
                                    print(f"  🔗 Stacking SD detectado: ${my_sd:,.2f}")
                                    break
                            if stacking_bonus > 0:
                                break
                    score = min(score + stacking_bonus, 100)
            ...
            for setup_key, setup in pending_setups.items():
                ...
                if setup["choch_alerted"]:
                    ...
                    if not setup["sl_hit"]:
                        ...
                        # v37 Diferencia #6: Partials 50/30/20 del PDF
                        tp_config = [
                            ("sd_m1",  lvls["sd_m1"],  "-1 SD",   "💡 Mover SL a breakeven"),
                            ("sd_m2",  lvls["sd_m2"],  "-2 SD",   "💡 Cerrar 50% de la posición. Mover SL a breakeven."),
                            ("sd_m25", lvls["sd_m25"], "-2.5 SD", "💡 Cerrar 30% adicional (80% total cerrado). Mover SL a -1 SD."),
                            ("sd_m4",  lvls["sd_m4"],  "-4 SD",   "🏆 Cerrar runner (20% final). Posición completa cerrada."),
                        ]
                        ...
                        if sl_hit:
                            ...
                            for tp_key, tp_price, tp_label, tp_advice in tp_config:
                                ...
                                if tp_reached:
                                    ...
                                    # v37 Diferencia #9: Displacement check en -2.5
                                    if tp_key == "sd_m25":
                                        try:
                                            recent_15m = get_candles(sym, "15m", 5)
                                            if recent_15m:
                                                last_c = recent_15m[-1]
                                                c_body = abs(last_c[4] - last_c[1])  # close - open
                                                c_range = abs(last_c[2] - last_c[3])  # high - low
                                                c_vol = last_c[5]
                                                avg_v = sum(c[5] for c in recent_15m[:-1]) / max(len(recent_15m) - 1, 1)
                                                vol_ok = c_vol > avg_v * 1.5 if avg_v > 0 else False
                                                body_ok = c_body > c_range * 0.60 if c_range > 0 else False
                                                if vol_ok and body_ok:
                                                    send_telegram(
                                                        f"🚀 <b>DESPLAZAMIENTO FUERTE en -2.5 SD</b>\n"
                                                        f"{sym} {setup['label']}\n\n"
                                                        f"Vela con cuerpo grande + volumen alto.\n"
                                                        f"Alta probabilidad de alcanzar -4 SD (${lvls['sd_m4']:,.2f})\n\n"
                                                        f"💡 <i>Mantener runner (20%). Target: -4 SD.</i>"
                                                    )
                                                else:
                                                    send_telegram(
                                                        f"⚠️ <b>Agotamiento en -2.5 SD</b>\n"
                                                        f"{sym} {setup['label']}\n\n"
                                                        f"Vela débil / sin volumen en -2.5.\n\n"
                                                        f"💡 <i>Considerar cerrar todo (80% + runner).\n"
                                                        f"Probabilidad baja de alcanzar -4 SD.</i>"
                                                    )
                                        except Exception as e:
                                            print(f"  ⚠️ Error displacement check: {e}")
