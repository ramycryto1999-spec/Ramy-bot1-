...
def send_test_message():
    ...
    test_key = f"TEST_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    ...
    if SHEETS_ENABLED:
        try:
            log_detection(detection, levels, score, test_key)
            print("✅ Test Sheets — fila creada")
        except Exception as e:
            print(f"❌ Test Sheets error alerta 1: {e}")
    ...
    if SHEETS_ENABLED:
        try:
            update_choch(test_key, detection["swing_choch"], confluences, price)
            print("✅ Test Sheets — fila actualizada con CHoCH")
        except Exception as e:
            print(f"❌ Test Sheets error alerta 2: {e}")
