...
def detect_manipulation(candles, interval):
    ...
    # Swing previo a la manipulacion para CHoCH 15M
    # Es el ultimo high/low relevante ANTES del spike
    if manip_type == "BAJISTA":
        # Para short: buscamos el ultimo swing low antes del spike
        swing_choch = min(c["low"] for c in prev_candles[-5:])
    else:
        # Para long: buscamos el ultimo swing high antes del spike
    ...
    return {
        ...
        "returned":      returned,
        "has_liq":       has_liq,
        "swing_choch":   swing_choch   # nivel a romper en 15M para confirmar
    }

# ─── CHoCH EN 15M ────────────────────────────────────────────
def check_choch_15m(symbol, detection):
    """
    Verifica si en 15M el precio cerro una vela por encima/debajo
    del swing previo a la manipulacion — eso confirma la inversion.
    """
    ...
    # Solo miramos velas posteriores a la manipulacion
    ...
    for c in post:
        if manip_type == "BAJISTA":
            # Para short: precio cierra POR DEBAJO del swing low previo
            if c["close"] < swing:
                return True, c["close"]
        else:
            # Para long: precio cierra POR ENCIMA del swing high previo
            if c["close"] > swing:
                return True, c["close"]
...
def calculate_levels(detection):
    c        = detection["candle"]
    mt       = detection["type"]
    fib_high = c["high"]   # nivel 0
    fib_low  = c["low"]    # nivel 1
    rango    = fib_high - fib_low

    if mt == "BAJISTA":
        # 1 en High, 0 en Low — SDs van hacia abajo
        ...
        # 1 en Low, 0 en High — SDs van hacia arriba
...
📏 <b>SD Projections:</b>
  ...
  -2 SD:   ${levels['sd_m2']:,.2f} — TP1
  -2.5 SD: ${levels['sd_m25']:,.2f} — TP2
...
📊 <b>CONTEXTO AMD:</b>
  Rango: ${detection['acc_low']:,.2f} — ${detection['acc_high']:,.2f}
...
def send_test_message():
    ...
    send_telegram("🧪 <b>TEST v10 — Alerta 1: manipulacion detectada</b>\n\n" +
                  format_detection_alert("BTCUSDT", "4H", detection, levels, score))
    time.sleep(2)
    send_telegram("🧪 <b>TEST v10 — Alerta 2: CHoCH 15M confirmado</b>\n\n" +
                  format_choch_alert("BTCUSDT", "4H", detection, levels, confluences,
                                     "BEARISH", "BEARISH", score,
                                     detection["swing_choch"], price * 0.999))
    print("✅ Test v10 enviado")

# ─── MAIN ────────────────────────────────────────────────────
def main():
    print("🤖 Bot TTrades v10 — CHoCH 15M como confirmacion de entrada")
    ...
    send_telegram(
        "🤖 <b>Bot TTrades AMD v10 activado</b>\n\n"
        "Sistema de 2 alertas:\n"
        "1️⃣ Manipulacion detectada — con niveles SD y swing a romper\n"
        "2️⃣ CHoCH 15M confirmado — entrada ahora con setup completo\n\n"
    ...
    while True:
        ...
        for sym in SYMBOLS:
            ...
            for tf in TIMEFRAMES:
                ...
                try:
                    ...
                    if setup_key not in pending_setups:
                        ...
                        send_telegram(msg)
                        print(f"  ✅ Alerta 1 — {label} {sym} esperando CHoCH 15M en ${detection['swing_choch']:,.2f}")
            ...
            # ── PASO 2: vigilar CHoCH 15M en setups pendientes ──
            ...
            for setup_key, setup in pending_setups.items():
                ...
                if age > SETUP_EXPIRY_H:
                    ...
                    send_telegram(
                        ...
                        f"Swing nivel: ${setup['detection']['swing_choch']:,.2f}\n"
                        f"Setup descartado."
                    )
                    continue
                ...
                if choch_confirmed and current_price:
                    ...
                    print(f"  🎯 Alerta 2 CHoCH — {sym} confirmado en ${choch_price:,.2f}")
                else:
